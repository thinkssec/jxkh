        ...
        public IList<KhArticlesModel> GetList()
        {
            ...
            if (list == null || list.Count == 0)
            {
                using (ORMDataAccess<KhArticlesData> db = new ORMDataAccess<KhArticlesData>())
                {
                    ...
                    {
                        //数据存入缓存系统
                        CacheHelper.Add(typeof(KhArticlesData), false, null, "GetList", null, CacheClassKey + "_GetList", list);
                    }
                }
            }
            return list;
        ...
        public IList<KhArticlesModel> GetListByHQL(string hql)
        {
            ...
            if (list == null || list.Count == 0)
            {
                using (ORMDataAccess<KhArticlesData> db = new ORMDataAccess<KhArticlesData>())
                {
                    ...
                    {
                        //数据存入缓存系统
                        CacheHelper.Add(typeof(KhArticlesData), false, null, "GetListByHQL", new object[] { hql }, CacheClassKey + "_GetListByHQL_" + hql, list);
                    }
                }
            }
            return list;
