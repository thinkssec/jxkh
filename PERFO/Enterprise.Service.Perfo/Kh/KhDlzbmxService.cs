...
namespace Enterprise.Service.Perfo.Kh
{
    ...
    public class KhDlzbmxService
    {
        ...
        public IList<KhDlzbmxModel> GetChildren(string khid, string jgbm,string father)
        {
            string sql = "select * from PERFO_KH_DLZBMX t where exists (select 1 from perfo_zbk_lhzb where parentzbbm='" + father + "' and lhzbbm=t.zbbm) and khid='" + khid + "' and jgbm='" + jgbm + "' ";
        ...
        public void CalDlzbmxSjdfAndSave(List<KhDlzbmxModel> dlzbmxLst, bool isCalculateAll)
        {
            ...
            foreach (var zb in mainZbLst)
            {
                if (!string.IsNullOrEmpty(zb.ZbsxModel.JxzrsZb.ZJSGXS))
                {
                    ...
                    KhDlzbmxModel m = calSubZhibiaoDF(zb, subZbList, dlzbmxLst, jsgzList, isCalculateAll);
                    //存入集合
                    m.DB_Option_Action = WebKeys.UpdateAction;
                    if (!finishList.Exists(p => p.ID == m.ID))
                        finishList.Add(m);
            ...
            List<KhDlzbmxModel> allZhibiaoLst, List<ZbkJsgzModel> jsgzList, bool isCalcFzzb)
        {
            ...
            foreach (var zb in subZhibiaoLst)
            {
                if (!string.IsNullOrEmpty(zb.ZbsxModel.JxzrsZb.ZJSGXS))
                {
                    ...
                    //if(zb.ZBBM=="LHZB20150923234238115"){
                    //    var l = subZbList;
                    //}
                    KhDlzbmxModel m = calSubZhibiaoDF(zb, subZbList, allZhibiaoLst, jsgzList, isCalcFzzb);
                    //替换表达式里的代号
            ...
            if (calV != null)
            {
                ...
                //end
                if (calV == null) model.SJDF = 0;
                else model.SJDF = calV;
