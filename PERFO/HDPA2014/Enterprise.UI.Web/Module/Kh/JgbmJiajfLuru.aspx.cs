...
namespace Enterprise.UI.Web.Kh
{
    ...
    ///机关部门加减分录入页面
    /// </summary>
    public partial class JgbmJiajfLuru : PageBase
    {
        ...
        protected void BindGrid() 
        {
            ...
            List<KhDfzbmxModel> dfzbList = dfzbmxSrv.GetListByKhidAndZhibiao(Ddl_Kaohe.SelectedValue,
                Ddl_Zhibiao.SelectedValue, WebKeys.KaoheType.机关部门) as List<KhDfzbmxModel>;
        ...
        protected void BindDdl()
        {
            ...
            //指标
            var zhibiaoLst = dfzbmxSrv.GetListByKhidAndKhdx(Ddl_Kaohe.SelectedValue, WebKeys.KaoheType.机关部门);
            zhibiaoLst = zhibiaoLst.Where(p => p.ZbsxModel.JxzrsZb.Dfzb.Zbxx.YJZBMC.Contains("加减分") 
                || p.ZbsxModel.JxzrsZb.Dfzb.Zbxx.YJZBMC.Contains("费用控制")).DistinctBy(p => p.ZBBM).ToList();//一个指标只取一条
        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (Pnl_Edit.Visible)
                {
                    ...
                    //打分结果 7
                    e.Row.Cells[7].Text = model.SJDF.ToRequestString();
                }
                else
                {
                    //打分值 5
        ...
        protected void LnkBtn_Upd_Click(object sender, EventArgs e)
        {
            ...
            List<KhDfzbmxModel> dfzbmxLst = dfzbmxSrv.GetListByKhid(Ddl_Kaohe.SelectedValue) as List<KhDfzbmxModel>;
            foreach (GridViewRow gvr in GridView1.Rows)
            {
                //DFZBID
                string DFZBID = GridView1.DataKeys[gvr.RowIndex].Values["DFZBID"].ToRequestString();
                KhDfzbmxModel model = dfzbmxLst.FirstOrDefault(p=>p.DFZBID == DFZBID);//dfzbmxSrv.GetSingle(Ddl_Kaohe.SelectedValue, DFZBID);
                if (model == null)
                    continue;
        ...
        private string getDafeiQujian(ZbkDfzbModel dfzb)
        {
            ...
            if (dfzb.MAXV != null && dfzb.MINV != null)
            {
                dfqj = " max=\"" + dfzb.MAXV.Value + "\" min=\"" + dfzb.MINV.Value + "\" ";
            }
            else
            {
                switch (dfzb.PFLX)
