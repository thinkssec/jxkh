...
namespace Enterprise.UI.Web.Kh
{
    ...
    ///机关部门考核上级打分页面
    /// </summary>
    public partial class JgbmWczShangji : PageBase
    {
        ...
        protected void GridView2_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                e.Row.Cells[1].Text = string.Format("<a href='{0}'>{1}</a>",
                    GetPageUrlAndParams("?" + url).TrimStart("~".ToCharArray()), SysBmjgService.GetBmjgName(model.JGBM.Value));
                //测评状态 2
        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (supperM.GetType().IsAssignableFrom(typeof(KhDlzbmxModel)))
                {
                    ...
                    var sjcpDfmodel = model.KhJgbmdfbLst.FirstOrDefault(p => p.DFZ == userModel.LOGINID);
                    if (sjcpDfmodel != null)
                    {
                        ...
                        if (Pnl_Edit.Visible == true)
                        {
                            ...
                            e.Row.Cells[5].Text =
                                ...
                                + sjcpDfmodel.WCJD.ToInt() + "',tipFormatter: function(value){return value+'%';},onChange: function(value){var standVal='" + model.ZbsxModel.SXFZ.ToDecimal() + "';$('#" + txtId5 + "').val(value);$('#" + txtId6 + "').numberbox('setValue', Math.round(value/10*standVal)/10);}\"/>"
                                + "<input type='hidden' id='" + txtId5 + "' name='" + txtId5 + "' value='" + sjcpDfmodel.WCJD + "'/>";
                            //测评得分6
                            e.Row.Cells[6].Text = Utility.GetTextBox(txtId6, sjcpDfmodel.KHDF, 6, (e.Row.RowIndex + 1), "number", true,
                            ...
                            e.Row.Cells[7].Text = Utility.GetTextBox("Txt" + 7 + "_" + (e.Row.RowIndex + 1),
                                sjcpDfmodel.DFSM, 7, (e.Row.RowIndex + 1), "string", false, "", "width:220px;");
                        }
                        else
                        {
                            //完成率 5
                            e.Row.Cells[5].Text = (sjcpDfmodel.WCJD != null) ? sjcpDfmodel.WCJD.Value + "%" : "";
                            //测评得分6
                            e.Row.Cells[6].Text = sjcpDfmodel.KHDF.ToRequestString();
                            //测评说明7
                            e.Row.Cells[7].Text = sjcpDfmodel.DFSM;
                            e.Row.Cells[7].ToolTip = "打分日期：" + sjcpDfmodel.DFSJ.ToDateYMDFormat();
                        }

                        HjDefen += sjcpDfmodel.KHDF.ToDecimal();
                ...
                else if (supperM.GetType().IsAssignableFrom(typeof(KhDfzbmxModel)))
                {
                    ...
                    var sjcpDfmodel = dfModel.KhJgbmdfbLst.FirstOrDefault(p => p.DFZ == userModel.LOGINID);
                    if (sjcpDfmodel != null)
                    {
                        img.ImageUrl = "/Resources/Images/yes.png";//有权限
                        if (Pnl_Edit.Visible == true)
                        {
                            //测评得分6
                            e.Row.Cells[6].Text = getHiddenInputAndStarRaty(6 + "_" + (e.Row.RowIndex + 1), sjcpDfmodel.KHDF.ToDecimal());
                            //测评说明7
                            e.Row.Cells[7].Text = Utility.GetTextBox("Txt" + 7 + "_" + (e.Row.RowIndex + 1),
                                sjcpDfmodel.DFSM, 7, (e.Row.RowIndex + 1), "string", false, "", "width:220px;");
                        }
        ...
        protected void LnkBtn_Upd_Click(object sender, EventArgs e)
        {
            ...
            foreach (GridViewRow gvr in GridView1.Rows)
            {
                ...
                if (ID.StartsWith("LH"))
                {
                    ...
                    var dfzModel = model.KhJgbmdfbLst.FirstOrDefault(p => p.DFZ == userModel.LOGINID);
                    if (dfzModel != null)
                    {
