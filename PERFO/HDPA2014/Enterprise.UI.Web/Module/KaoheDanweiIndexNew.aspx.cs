...
namespace Enterprise.UI.Web.Module
{
    ...
    /// 绩效考核首页面--二级单位用
    /// </summary>
    public partial class KaoheDanweiIndexNew : PageBase
    {
        ...
        /// 二级单位考核得分服务类
        /// </summary>
        protected KhEjdwkhdfService ejdwkhdfSrv = new KhEjdwkhdfService();
        ...
        private void InitData()
        {
            ...
            KaoheLst = khglSrv.GetKhListByLxidForIndex("LX2014A") as List<KhKhglModel>;
        ...
        protected void GridView_Kaohe_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                //单位得分 4
                EjdwkhdfList = ejdwkhdfSrv.GetEjdwdfListByKhid(model.KHID.ToString()).Where(p => !string.IsNullOrEmpty(p.HZBZ)).ToList();
                var bmdf = EjdwkhdfList.FirstOrDefault(p => p.KHLX == ((int)WebKeys.KaoheType.二级单位).ToString() && p.JGBM == userModel.JGBM);
                if (bmdf != null)
                {
                    e.Row.Cells[4].Text = "<div class=\"show\"><span class=\"blue\">" + bmdf.DWZDF + "</span></div>";
                }
                //领导班子得分 5
                var bmfzrdf = EjdwkhdfList.FirstOrDefault(p => p.KHLX == ((int)WebKeys.KaoheType.领导班子).ToString() && p.JGBM == userModel.JGBM);
                if (bmfzrdf != null)
                {
        ...
        protected string GetEjdwKaoheInfo()
        {
            ...
            {
                khInfo = (string)CacheHelper.GetCache(khglSrv.CacheKey + "_GetErjidanweiKaoheInfo_" + userModel.JGBM);
            }
            ...
            {
                //数据存入缓存系统
                CacheHelper.Add(khglSrv.CacheKey + "_GetErjidanweiKaoheInfo_" + userModel.JGBM, khInfo);
            }
            return khInfo;
        }
        ...
        protected string GetChartData(int type)
        {
            ...
            {
                EjdwkhdfList = ejdwkhdfSrv.GetListByKhid(kh.KHID.ToString()).
                ...
                switch (type)
                {
                    case 1://单位得分
                        ...
                        if (bmdf != null)
                        {
                            datas.Add(bmdf.DWZDF.ToDecimal().ToString());
                        }
                        break;
                    case 2://领导班子得分
                        var bmfzrdf = EjdwkhdfList.FirstOrDefault(p => p.KHLX == ((int)WebKeys.KaoheType.领导班子).ToString() && p.JGBM == userModel.JGBM);
