...
namespace Enterprise.UI.Web.Kh
{
    ...
    /// 发起考核页面
    /// </summary>
    public partial class KhJgbmldAudit : PageBase
    {
        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (model.STATUS != "30")
                    ib.Enabled = false;
                e.Row.Cells[4].Text = mbjgSrv.GetStatus(model);
                //考核类型
        ...
        {
            Response.Redirect("~/Module/Kh/KhJgbmldshList.aspx", true);
        }
        ...
        protected void Btn_Ok_Click(object sender, EventArgs e)
        {
            ...
            foreach (GridViewRow row in GridView1.Rows)
            {
                ...
                if (mbjgSrv.Execute(md)) {
                    List<SysUserModel> s = usrv.GetList().Where(p => p.DUTY == "基层机关"&&p.JGBM==md.JGBM).ToList();
                    if (s != null) { 
                        foreach(var u in s){
                            user.Add(u.LOGINID);
                        }
            ...
            if (user.Count > 0)
            {
                string fileUrl = "Module/Kh/JcdwKhdfList.asp";
