...
namespace Enterprise.UI.Web.Kh
{
    ...
    ///机关部门指标筛选维护页面
    /// </summary>
    public partial class JcdwKhdfList2 : PageBase
    {
        ...
        public List<KhXsModel> xsL = new List<KhXsModel>();
        ...
        public string Khid = (string)Utility.sink("KHID", Utility.MethodType.Get, 0, 0, Utility.DataType.Str);//考核ID
        ...
        protected void BindGrid() 
        {
            ...
            jgL = jgSrv.GetList().Where(p => p.JGLX == "基层单位"&&p.SFKH=="1"&&p.JGBM==this.userModel.Bmjg.JGBM).OrderBy(o=>o.PX).ToList();
            ...
            {
                if (!dic.ContainsKey(r["ID"].ToRequestString() + r["JGBM"].ToRequestString()))
            ...
            sql = @"select t.khdzbid,t.id
                     ...
                     where a.khid = b.khid
                     and b.id=t.khdzbid
                     and a.khid=" + Khid + " and t.jgbm=" + this.userModel.JGBM;
            ds = khSrv.getDsBySql(sql);
            foreach (DataRow r in ds.Tables[0].Rows)
            {
                if (!dic3.ContainsKey(r["KHDZBID"].ToRequestString()))
                dic3.Add(r["KHDZBID"].ToRequestString(), r["ID"].ToRequestString());
            }
            var l1 = qzSrv.GetList().Where(p => p.KHID == Hid_KHID.Value.ToInt() && p.JGBM == this.userModel.Bmjg.JGBM).ToList();
            foreach (var m in l1)
            {
                if (!glc.ContainsKey(m.KHDZBID))
                glc.Add(m.KHDZBID, m.QZ == null ? 0 : m.QZ.Value);
            }


            sql="select a.* from PERFO_KH_XS a ,perfo_sys_bmjg b where a.deptid=b.JGBM and b.jgbm="+this.userModel.Bmjg.JGBM+" order by b.px";
            xsL = xsSrv.GetListBySQL(sql).ToList();

            sql = "select t.* from PERFO_KHS_MBJG t,perfo_khs_khdzb a,perfo_kh_khgl b where b.khid="+Khid+" and a.khid=b.khid and t.khdzbid=a.id and t.jgbm="+this.userModel.JGBM;
            List<KhsMbjgModel> mbLs = mbjgSrv.GetListBySQL(sql).ToList();
            foreach (var m in mbLs)
            {
                if (!mbjg.ContainsKey(m.KHDZBID))
        ...
        protected void LnkBtn_Ins_Click(object sender, EventArgs e)
        {
            ...
            KhsKhdzbService dzbkhSrv = new KhsKhdzbService();
            KhsQzService qzSrv=new KhsQzService();
            jgL = jgSrv.GetList().Where(p => p.JGLX == "基层单位").OrderBy(o => o.PX).ToList();
            dzbL = dzbkhSrv.GetList().Where(p => p.KHID == Khid.ToInt()).OrderBy(o => o.PX).ToList();
            int j = 0;
            foreach (var m in dzbL) {
                ...
                foreach (var jg in jgL) {
                    ...
                    if (i < 6)
                    {
                        ...
                        if (!string.IsNullOrEmpty(wqxqz[j]))
                            n.QZ = wqxqz[j].ToDecimal();
                    }
                    else if (i < 9 && i > 7)
                    {
                        if (!string.IsNullOrEmpty(jtqz[j]))
                            n.QZ = jtqz[j].ToDecimal();
                    }
                    else if (i < 10 && i > 8)
                    {
                        if (!string.IsNullOrEmpty(ysjqz[j]))
                            n.QZ = ysjqz[j].ToDecimal();
                    }
                    qzSrv.Execute(n);
                    i++;
                }
        ...
        protected void LnkBtn_Cancel_Click(object sender, EventArgs e)
        {
            ...
            Response.Redirect("~/Module/Kh/JcdwKhdfList.aspx", true);
        }
        ...
        protected void Btn_Ok_Click(object sender, EventArgs e)
        {
            ...
            SysUserModel s = usrv.GetList().Where(p => p.DUTY == "绩效考核办公室").FirstOrDefault();
            if (s != null)
            {
                user.Add(s.LOGINID);
            }
            if (user.Count > 0)
            {
                string fileUrl = "Module/Kh/KhJxkhbshList.asp";
                ...
                msgService.CloseMessage(this.userModel, khm.KHID, module.MID, this.userModel.LOGINID);
                msgService.SendMsgToUers(user, 1, khm.KHID + "",
                    "绩效考核办公室审核","【"+SysBmjgService.GetBmjgName(this.userModel.JGBM)+ "】需要您审核【" + khm.KHMC + "】得分!",
                    "/Module/Kh/KhJxkhbshList.aspx?jgbm=" + this.userModel.JGBM,
