...
namespace Enterprise.UI.Web.Kh
{
    ...
    public partial class EjdwWczShenhe : PageBase
    {
                    ...
                    if (model.WCZSDRQ == null)
                    {
                        ...
                        if (Pnl_Edit.Visible == true)
                        {
                            ...
                            if (child.Count == 0)
                            {
                                ...
                                e.Row.Cells[4].Text = "<strong>"+model.MBZ.ToRequestString()+"</strong>";
                                e.Row.Cells[7].Text = model.WCZSHZ.ToRequestString();
                                e.Row.Cells[8].Text = model.WCZSHBZ.ToRequestString();
                                e.Row.Cells[9].Text = "<strong>" + model.SJDF.ToRequestString() + "</strong>";
        ...
        protected void LnkBtn_Ins_Click(object sender, EventArgs e)
        {
            ...
            //如果标志位不为空，则要全部重新计算
            bool isCalculateAll = (!string.IsNullOrEmpty(Hid_Recount.Value));
            //3==计算关联指标的完成值
            foreach (var dlzb in khdlzbList)
            {
                if (dlzb.LhzbjcsjLst.Count > 0 && isCalculateAll)
                {
                    //有关联指标，则要代入关系式计算完成值
            ...
            dlzbmxSrv.CalDlzbmxSjdfAndSave(khdlzbList, isCalculateAll);
