...
namespace Enterprise.UI.Web.Kh
{
    ...
    public partial class JgbmWczShending : PageBase
    {
                ...
                if (jgzfjx != null)
                {
                    if (Pnl_Edit.Visible)
                    {
                        e.Row.Cells[4].Text =
                            ...
                            "number", true, "", "width:80px;");
                        e.Row.Cells[5].Text =
                            Utility.GetTextBox("Txt_" + 5 + "_" + (e.Row.RowIndex + 1), jgzfjx.KHDF, 0, 0,
                            "number", true, "", "width:80px;");
                    }
                    else
                    {
                        ...
                        e.Row.Cells[5].Text = string.Format("<a href=\"javascript:parent.addTab('/M.K.JgbmJgzfHz?KH={0}','作风建设打分汇总');\">{1}</a>", jgzfjx.KHID, jgzfjx.KHDF);
                    }
                }
        ...
        protected void LnkBtn_Upd_Click(object sender, EventArgs e)
        {
            ...
            //}
            //3==保存数据
            if (jgbmkhdfSrv.ExecuteSQL("begin "+sqlSB.ToString()+" null;end;"))
            {
                ...
                if (v > 0)
                {
                    ...
                    if (jgbmkhdfSrv.ExecuteSQL("begin "+sql+" null;end;"))
                    {
                        //完成考核
