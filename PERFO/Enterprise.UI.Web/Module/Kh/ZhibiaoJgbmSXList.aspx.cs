...
namespace Enterprise.UI.Web.Kh
{
    ...
    /// 机关部门考核指标筛选维护页面
    /// </summary>
    public partial class ZhibiaoJgbmSXList : PageBase
    {
        ...
        {
            //单位
            Ddl_Danwei.DataSource = bmjgService.GetBmjgTreeLst(false).Where(p => !p.JGLX.Contains("基层")).ToList();
        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                //分值合计 6
                e.Row.Cells[6].Text = ZbsxList.Where(p => p.SXJGBM == model.JGBM && p.JxzrsZb.ZZBXZ == "主指标").Sum(p => p.SXFZ).ToRequestString();
            ...
            switch (e.CommandName)
            {
                case "bianji":
                    if (bm > 0)
                    {
                        ...
                        Response.Redirect("~/Module/Kh/ZhibiaoJgbmSXEdit.aspx?" + url, true);
                    }
                    break;
                case "shanchu":
                    if (bm > 0)
