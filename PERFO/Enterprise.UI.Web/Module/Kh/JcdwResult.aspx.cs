...
namespace Enterprise.UI.Web.Kh
{
    ...
    ///机关部门指标筛选维护页面
    /// </summary>
    public partial class JcdwResult : PageBase
    {
        ...
        public Dictionary<string, decimal> glc2 = new Dictionary<string, decimal>();
        ...
        protected void BindGrid() 
        {
            ...
            jgL = jgSrv.GetList().Where(p => p.JGLX == "基层单位"&&p.SFKH=="1").OrderBy(o=>o.PX).ToList();
            ...
            var l1 = qzSrv.GetList().Where(p=>p.KHID==Hid_KHID.Value.ToInt()&&p.JGBM==2).ToList();
            foreach (var m in l1)
            {
                glc.Add(m.KHDZBID, m.QZ == null ? 0 : m.QZ.Value);
            }

            var l12 = qzSrv.GetList().Where(p => p.KHID == Hid_KHID.Value.ToInt() && p.JGBM == 63).ToList();
            foreach (var m in l12)
            {
                glc2.Add(m.KHDZBID, m.QZ == null ? 0 : m.QZ.Value);
            }

            var l2 = qzSrv.GetList().Where(p => p.KHID == Hid_KHID.Value.ToInt() && p.JGBM == 104).ToList();
            foreach (var m in l2)
            {
                wqx.Add(m.KHDZBID, m.QZ==null?0:m.QZ.Value);
            }

            var l3 = qzSrv.GetList().Where(p=>p.KHID==Hid_KHID.Value.ToInt()&&p.JGBM==153).ToList();
            foreach (var m in l3)
            {
                jt.Add(m.KHDZBID, m.QZ == null ? 0 : m.QZ.Value);
            }
            var l4 = qzSrv.GetList().Where(p => p.KHID == Hid_KHID.Value.ToInt() && p.JGBM == 280).ToList();
            foreach (var m in l4)
            {
                ysj.Add(m.KHDZBID, m.QZ == null ? 0 : m.QZ.Value);
            }

            sql="select a.* from PERFO_KH_XS a ,perfo_sys_bmjg b where a.deptid=b.JGBM order by b.px";
        ...
        protected void LnkBtn_Ins_Click(object sender, EventArgs e)
        {
            ...
            string[] glcqz2 = Request["glcqz2"].ToRequestString().Split(',');
            ...
            foreach (var m in dzbL) {
                ...
                foreach (var jg in jgL) {
                    ...
                    if (i < 3)
                    {
                        ...
                        if (!string.IsNullOrEmpty(glcqz2[j]))
                            n.QZ = glcqz2[j].ToDecimal();
                    }
                    else if (i < 8 && i > 5)
                    {
        ...
        protected void LnkBtn_Cancel_Click(object sender, EventArgs e)
        {
            ...
            Response.Redirect("~/Module/Kh/JcdwList.aspx", true);
        }
