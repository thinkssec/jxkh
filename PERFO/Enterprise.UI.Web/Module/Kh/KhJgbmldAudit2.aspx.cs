...
namespace Enterprise.UI.Web.Kh
{
    ...
    /// 发起考核页面
    /// </summary>
    public partial class KhJgbmldAudit2 : PageBase
    {
        ...
        //SysFgbmjgService fgjgSrv = new SysFgbmjgService();
        #region 权限检查
...
//            string sql = @"select d.zbmc key1, e.jgmc key2, a.khid key3, c.*
...
//                               and b.dzbid = d.dzbid
//                               and c.jgbm = e.jgbm
//                               and d.fzbm = '"+this.userModel.JGBM+"' and a.khid="+khId;
            
//            GridView1.DataSource = mbjgSrv.GetListBySQL(sql).ToList();
//            GridView1.DataBind();
            ...
            string sql = @"select d.zbmc key1, e.jgmc key2, a.khid key3, c.*
                             ...
                             where a.khid = b.khid
                               ...
                               and b.dzbid = d.dzbid
                               and c.jgbm = e.jgbm
                               and d.fzbm = '" + this.userModel.JGBM + "' and a.khid=" + khId + " and c.jgbm<>" + this.userModel.JGBM + "";
            GridView1.DataSource = mbjgSrv.GetListBySQL(sql).ToList();
            GridView1.DataBind();
        }
        ...
        {
            Response.Redirect("~/Module/Kh/KhJgbmldshList2.aspx", true);
        }
        ...
        protected void Btn_Ok_Click(object sender, EventArgs e)
        {
            ...
            {
                CheckBox ckid = (CheckBox)row.Cells[1].FindControl("Cb_ID");
                ...
                if (mbjgSrv.Execute(md)) {
                    string sss = @"select a.* from PERFO_SYS_USER a,PERFO_SYS_FGBMJG b where a.loginid=b.loginid and b.jgbm=" + md.JGBM;
                    //List<SysFgbmjgModel> fg = fgjgSrv.GetList().Where(p => p.JGBM == md.JGBM).ToList();
                    List<SysUserModel> s = usrv.GetListBySQL(sss).ToList();

                    ////List<SysUserModel> s = usrv.GetList().Where(p => p.DUTY == "专业部门分管领导" && p.JGBM == md.JGBM).ToList();
                    if (s != null) { 
                        foreach(var u in s){
                            user.Add(u.LOGINID);
                        }
            ...
            if (user.Count > 0)
            {
                string fileUrl = "Module/Kh/JgfgKhdfList.aspx";
                ...
                msgService.SendMsgToUers(user, 1, khm.KHID + "",
                    "专业分管领导确认", "需要您确认【" + khm.KHMC + "】得分!",
                    "Module/Kh/JgfgKhdfList.aspx",
        ...
        protected void Btn_No_Click(object sender, EventArgs e)
        {
            ...
            //if (user.Count > 0)
            //{
            //    string fileUrl = "Module/Kh/KhJcdwkhqrList.asp";
            //    SysModuleService moduleService = new SysModuleService();
            //    KhMessageService msgService = new KhMessageService();
            //    IList<SysModuleModel> moduleLst = moduleService.GetList();
            //    SysModuleModel module = moduleLst.FirstOrDefault(p => p.MURL.ToUpper().Contains(fileUrl.ToUpper()) || p.WEBURL.ToUpper() == fileUrl.ToUpper());
            //    msgService.CloseMessage(this.userModel, khm.KHID, module.MID, this.userModel.LOGINID);
            //    msgService.SendMsgToUers(user, 1, khm.KHID + "",
            //        "基层单位考核确认", "需要您确认【" + khm.KHMC + "】得分!",
            //        "/Module/Kh/KhJcdwkhqrList.aspx",
            //        module.MID, this.userModel.LOGINID);
            //}
