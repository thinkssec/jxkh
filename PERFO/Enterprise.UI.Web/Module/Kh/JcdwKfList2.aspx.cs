...
namespace Enterprise.UI.Web.Kh
{
    ...
    ///机关部门指标筛选维护页面
    /// </summary>
    public partial class JcdwKfList2 : PageBase
    {
...
//                                     where c.khid = "+Khid+" group by c.id, d.jgbm, c.px, c.dzbid order by c.px");
            ...
            //string sql = string.Format(@"select a.khdzbid id,a.jgbm,ms from PERFO_KHX_RESULT t,perfo_khs_mbjg a where t.mbjgid=a.id and t.khid=" + Khid + " order by a.khdzbid");
            string sql = string.Format(@"select * from vw_kouf where khid=" + Khid);
            ...
            foreach (DataRow r in ds.Tables[0].Rows)
            {
                ...
                {
                    if (r["MS"] != null && !string.IsNullOrEmpty(r["MS"].ToRequestString()))
                    {
                        jj = 1;
                        dic.Add(r["ID"].ToRequestString() + r["JGBM"].ToRequestString(), jj + "、" + r["MS"].ToRequestString() + "</br>");
                    }
                }
                else {
                    if (r["MS"] != null && !string.IsNullOrEmpty(r["MS"].ToRequestString()))
                    {
                        jj++;
                        string last = dic[r["ID"].ToRequestString() + r["JGBM"].ToRequestString()] + jj + "、" + r["MS"].ToRequestString() + "</br>";
                        dic[r["ID"].ToRequestString() + r["JGBM"].ToRequestString()]=last;
                    }
        ...
        protected void LnkBtn_Cancel_Click(object sender, EventArgs e)
        {
            ...
            Response.Redirect("~/Module/Kh/JcdwKfList.aspx", true);
        }
