...
namespace Enterprise.UI.Web.Kh
{
    ...
    /// 发起考核页面
    /// </summary>
    public partial class KhxfList : PageBase
    {
        ...
        protected void BindGrid() 
        {
            ...
            //GridView1.DataSource = list.Where(p=>p.KHZT=="0"||p.KHZT=="-1").ToList();
            GridView1.DataSource = list.Where(p => p.LXID != "LX2014C").OrderByDescending(o => o.KHID).ToList();
            GridView1.DataBind();
        ...
        protected void BindDdl()
        {
            ...
            Ddl_LXID.DataSource = kindSrv.GetList().Where(p => p.LXID != "LX2014C").ToList();
        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                ImageButton ib = (ImageButton)e.Row.Cells[6].FindControl("ImageButton2");
                if(ib!=null)
                ib.Visible = haveDel; 
            }
        }
        ...
        protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            dzbList = dzbSrv.GetList().Where(p => p.ZBMC != "基层班站考核指标").OrderBy(o => o.PX).ToList();
                ...
                if (khglSrv.Execute(model))
                {
                    ...
                    foreach (GridViewRow row in GridView2.Rows)
                    {
                        ...
                        if (dzb.ZBMC == "机关服务满意度指标") { 
                            var s = usrv.GetList().Where(p =>  p.DUTY == "四级单位").FirstOrDefault();
                            if (s != null)
                                usersj.Add(s.LOGINID);
                        }
                        if (t != null)
                            user.Add(t.LOGINID);
                    }
                    if (user.Count > 0)
                    {
                        string fileUrl = "Module/Kh/KhbzList.asp";
                        ...
                        msgService.SendMsgToUers(user, 1, model.KHID+"",
                            "制定评分标准", "需要您制定【"+model.KHMC+"】的评分标准!",
                            "/Module/Kh/KhbzList.aspx",
