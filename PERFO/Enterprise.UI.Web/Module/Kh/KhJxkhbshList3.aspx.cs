...
namespace Enterprise.UI.Web.Kh
{
    ...
    ///机关部门指标筛选维护页面
    /// </summary>
    public partial class KhJxkhbshList3 : PageBase
    {
        ...
        public string dwdm = (string)Utility.sink("DWDM", Utility.MethodType.Get, 0, 0, Utility.DataType.Str);//考核ID
        ...
        protected void BindGrid() 
        {
            ...
            jgL = jgSrv.GetList().Where(p => p.JGLX == "基层单位" && p.SFKH == "1" && p.JGBM == dwdm.ToInt()).OrderBy(o => o.PX).ToList();
            ...
            var l1 = qzSrv.GetList().Where(p => p.KHID == Hid_KHID.Value.ToInt() && p.JGBM == dwdm.ToInt()).ToList();
            foreach (var m in l1)
            {
            ...
            sql="select a.* from PERFO_KH_XS a ,perfo_sys_bmjg b where a.deptid=b.JGBM and b.jgbm="+dwdm+" order by b.px";
        ...
        protected void LnkBtn_Cancel_Click(object sender, EventArgs e)
        {
            ...
            Response.Redirect("~/Module/Kh/KhJxkhbshList2.aspx?KHID="+Khid, true);
        }
