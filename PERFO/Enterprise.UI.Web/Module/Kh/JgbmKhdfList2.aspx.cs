...
namespace Enterprise.UI.Web.Kh
{
    ...
    ///机关部门指标筛选维护页面
    /// </summary>
    public partial class JgbmKhdfList2 : PageBase
    {
        ...
        protected void BindGrid() 
        {
            ...
            jgL = jgSrv.GetList().Where(p => p.JGLX == "职能部门"&&p.SFKH=="1"&&p.JGBM==this.userModel.Bmjg.JGBM).OrderBy(o => o.PX).ToList();
...
//                                     where c.khid = "+Khid+" group by c.id, d.jgbm, c.px, c.dzbid order by c.px");
            ...
            string sql = string.Format(@"select * from VW_SCORE where khid="+Khid +" and jgbm="+this.userModel.Bmjg.JGBM);
        ...
        protected void LnkBtn_Cancel_Click(object sender, EventArgs e)
        {
            ...
            Response.Redirect("~/Module/Kh/JgbmKhdfList.aspx", true);
        }
