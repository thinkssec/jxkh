<%@ Page Title="" Language="C#" MasterPageFile="../Project.Master" AutoEventWireup="true" ValidateRequest="false"
    CodeBehind="KhJxkhbshList3.aspx.cs" Inherits="Enterprise.UI.Web.Kh.KhJxkhbshList3" %>
            ...
            try {
                for(var i=1;i<=<%=dzbL.Count() %>;i++){
                    ...
                    for(var j=1;j<=<%=jgL.Count() %>;j++){
                        ...
                        if(isNaN(zj*1)) zj=0;
                        $("#hj_"+j).text((cur*1+fz*1).toFixed(2));
        ...
        <div class="main-gridview">
            ...
            <table class="GridViewStyle" cellspacing="0" rules="all" border="1" id="ProjectPH_GridView1" style="width: 100%; border-collapse: collapse;">
                <tr class="GridViewHeaderStyle">
                    ...
                    <th colspan="2"><%=Enterprise.Service.Perfo.Sys.SysBmjgService.GetBmjgName(dwdm.ToInt()) %></th>
                </tr>
                <tr class="GridViewHeaderStyle">
                    <th>评分</br>（K） </th>
                    <th>分值</br>W*K </th>
                ...
                <tr>
                    ...
                    <td nowrap align="center"><input type="checkbox" name="CB_DZB" value="<%=dic3.ContainsKey(m.ID)?dic3[m.ID]:"" %>" <%=mbjg.ContainsKey(m.ID)?(mbjg[m.ID]== "50" ? "" : "disabled"):"disabled" %>/><a href="/Module/KhX/Khzbpf.aspx?khid=<%=Khid %>&mbjgid=<%=dic3.ContainsKey(m.ID)?dic3[m.ID]:"" %>&back=KhJxkhbshList3&audit=1&dwdm=<%=dwdm %>" ><%=dic2[m.DZBID] %></a></td>
